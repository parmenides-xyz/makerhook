...
library SphericalSwapMath {
    ...
    /// @dev Helper to calculate boundary discriminant
    function calculateBoundaryDiscriminant(
        uint256 radiusBoundaryQ96,
        uint256 kBoundaryQ96,
        uint256 sqrtNumAssetsQ96
    ) private pure returns (uint256) {
        uint256 kMinusRBSqrtN = kBoundaryQ96 - FullMath.mulDiv(
            radiusBoundaryQ96, 
            sqrtNumAssetsQ96, 
            FixedPoint96.Q96
        );
        ...
        return FullMath.mulDiv(
            ...
            FullMath.mulDiv(kMinusRBSqrtN, kMinusRBSqrtN, FixedPoint96.Q96)
        );
    }

    /// @dev Helper to calculate derivative
    function calculateDerivative(
        uint256 projectionQ96,
        int256 orthogonalDiff,
        uint256 sqrtOrthogonal,
        uint256 newReserveOut,
        uint256 newSumReservesQ96,
        SphericalMath.PoolConstants memory poolConstants
    ) private pure returns (int256 fPrime) {
        // dP/dΔ = -1/√n
        int256 dP_dDelta = -int256(FullMath.mulDiv(
            FixedPoint96.Q96, 
            FixedPoint96.Q96, 
            poolConstants.sqrtNumAssetsQ96
        ));

        // df/dP = 2P
        int256 df_dP = 2 * int256(projectionQ96);

        // dW/dΔ = -2*newReserveOut + 2*newSumReserves/n
        int256 dW_dDelta = -2 * int256(newReserveOut) + 
                            2 * int256(FullMath.mulDiv(newSumReservesQ96, 1, poolConstants.numAssets));
    ...
    function evaluateInvariantAndDerivative(
            ...
            // Calculate in scope to reduce stack depth
            {
                // Calculate projection onto equal-price vector
                uint256 temp1 = FullMath.mulDiv(
                    newSumReservesQ96,
                    FixedPoint96.Q96,
                    params.poolConstants.sqrtNumAssetsQ96
                );
                uint256 temp2 = FullMath.mulDiv(
                    params.radiusInteriorQ96,
                    params.poolConstants.sqrtNumAssetsQ96,
                    FixedPoint96.Q96
                );
                projectionQ96 = temp1 - params.kBoundaryQ96 - temp2;
            }
            
            // Calculate orthogonal components in scope
            {
                ...
                uint256 boundaryDiscriminantQ96 = calculateBoundaryDiscriminant(
                ...
                sqrtOrthogonal = SphericalMath.sqrt(orthogonalSqQ96);
                uint256 sqrtBoundaryDisc = SphericalMath.sqrt(boundaryDiscriminantQ96);
                orthogonalDiff = int256(sqrtOrthogonal) - int256(sqrtBoundaryDisc);
            }

            // Calculate f
            {
                uint256 projSq = FullMath.mulDiv(projectionQ96, projectionQ96, FixedPoint96.Q96);
                uint256 orthDiffSq = FullMath.mulDiv(uint256(abs(orthogonalDiff)), uint256(abs(orthogonalDiff)), FixedPoint96.Q96);
                uint256 radiusSq = FullMath.mulDiv(params.radiusInteriorQ96, params.radiusInteriorQ96, FixedPoint96.Q96);
